                ...
                <div>
                    ...
                    <select name="roomId" required>
                        <c:forEach items="${rooms}" var="room">
                            <option value="${room.id}">${room.roomNumber} - ${room.roomType} - ₹${room.pricePerNight}</option>
                        </c:forEach>
                    </select>
                </div>
                <div>
        ...
        <table>
            ...
            <c:forEach items="${bookings}" var="booking">
                <tr>
                    ...
                    <td>${booking.roomNumber}<br><small>${booking.roomType}</small></td>
                    ...
                    <td class="actions stacked">
                        <c:if test="${booking.status == 'RESERVED'}">
                            <form action="${pageContext.request.contextPath}/bookings" method="post">
                                ...
                                <button class="btn small" type="submit" name="action" value="checkin">Check In</button>
                                <button class="btn small warning" type="submit" name="action" value="cancel">Cancel</button>
                            </form>
                        </c:if>
                        <c:if test="${booking.status == 'CHECKED_IN'}">
                            <form action="${pageContext.request.contextPath}/bookings" method="post">
                                <input type="hidden" name="id" value="${booking.id}">
                                <button class="btn small" type="submit" name="action" value="checkout">Check Out</button>
                            </form>
                        </c:if>
                        <form action="${pageContext.request.contextPath}/bookings" method="post">
                            <input type="hidden" name="id" value="${booking.id}">
                            <button class="btn small danger" type="submit" name="action" value="delete" onclick="return confirm('Delete this booking?')">Delete</button>
                        </form>
                    </td>
                </tr>
            </c:forEach>
